...
- 画面から指定/アップロードするもの:
  ...
  2) マッピングマスタ（全体で1ファイル）
  3) 試算表ファイル（全体で1ファイル）
  4) 計上部門別に出力した総勘定元帳ファイル（支店ごとに1ファイル）。アップローダは `BRANCHES` の定義分だけ表示し、支店名付きで個別に指定する（例: 本社, 心xx店, …）。
...
### 7.4 表示マスク（UI/出力）［2025-09-04 追加］
- 目的: 本番リリース前の実名露出を避けるため、名称の「表示のみ」をマスクする（KISS/DRY）。
- 適用箇所:
  - Ledgerフォームの支店A/Bプルダウンの表示名
  - TB結果テーブルの支店名カラム
  - 元帳照合の出力Excel（`info` シート）に記録する支店名
- 入出力のキーは支店コード（`BRANCHES.code`）を正とし、ロジックは非マスク名に依存しない（DRY）。
- マスクルール:
  - 本社 → 本社（変更なし）
  - 法人外商 → 法人外xx
  - 「〜店」 → 先頭1文字 + "xx店"（例: 心斎橋店 → 心xx店、東京店 → 東xx店）
  - 上記以外 → 変更なし
- 実装: `lib/mask.ts` の `maskBranchName(name)` を通して表示のみ変換する。

### 7.5 AI分析（アンマッチ）［2025-09-04 追加］
- 目的: アンマッチが発生した場合に、人が次に着目すべきポイントを短時間で把握できる要約を提示（KISS）。
- トリガー: `ledgerReconcileAction` の結果 `meta.hasUnmatch = true`。
- 方式: サーバーアクションはアンマッチの最小ペイロード（期間、支店A/B、日別サマリ、アンマッチ行抜粋）を返却。クライアントが `/api/ai/ledger-unmatch` にPOSTしてAI要約を取得（DRY）。
- 表示: 実行ボタン直下のログの下に「AI分析」ボックスを追加。
  - 実行直後に「AI分析中…」ローディング表示。
  - 完了時に日本語の箇条書きサマリ（概要/主要ズレ/原因仮説/次の確認）。
  - エラー時は簡潔なエラー表示（ダウンロードフローは継続）。
- モデル/SDK: OpenAI SDK（Node）。既定モデル `gpt-4o-mini`。
...
- エラー時: 欠落列/壊れファイルで停止し、原因・ファイル名・列名を明示する。
 ...
 - AI分析: `hasUnmatch=true` のとき、
   - [ ] ログ下に「AI分析中…」が表示される。
   - [ ] 要約が日本語で表示される（概要/主要ズレ/原因仮説/次の確認）。
   - [ ] OPENAI_API_KEY 未設定時はエラー表示となる（ダウンロードは先行実行される）。
