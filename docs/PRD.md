...
### 3.1.0 詳細ログの抑止（2025-09-11 追加）
- 目的: `/balance-detail` 実行時に出力される詳細なマッチングログのノイズを低減する。
- 仕様: 以下のログはデフォルトで非出力。出力が必要なときは環境変数 `AUTOGROUP_VERBOSE_LOG=true` を設定。
  - 「部分一致: ...」
  - 「取引先XXXXのマッピング数: N」
  - 「科目YYYY: マッチN件, 未マッチM件」
  - 「科目ZZZZZ処理完了: プロジェクトN件, エントリM件」
  - 「部門XXXXの全科目処理完了: 検証済み科目数=N」
  - 対象実装: `app/actions/project-autogroup.ts`（`VERBOSE_AUTOGROUP_LOG` フラグで制御）
...
- 受入基準:
  ...
  - 2025-09-11 追記: `Entry一括挿入完了: N件` の直後に `setProgress(workflowId, 0, 1)` を設定し、UIステータスを `[3. AI分類処理中...]` へ即時遷移させる（その後、部門数に基づく正確な total へ上書き）。
  - 2025-09-11 追記(UX改善): クライアント側でも保存開始直後に `[3. AI分類処理中...]` へ遷移させ、ポーリングは`[4. データ読込中...]`への到達検知のみに使用する。これにより`[2. 仕訳データ保存中...]`から`[4. データ読込中...]`へ飛ぶ事象を回避。

更新履歴:
- 2025-09-09: ログ方針を追加。Console/Prismaのファイルミラーリングを導入。
 - 2025-09-09: ログ文言の日本語化/IDマスキング、進捗ポーリングを追加。
 - 2025-09-11: 自動グループ化の詳細マッチングログをデフォルト抑止（`AUTOGROUP_VERBOSE_LOG`）。
