...
- 案件削除（右クリックメニュー）
- 案件の展開/折りたたみ（三角アイコンクリック）
- リアルタイム金額計算・表示

#### F004: Excel 出力機能

- 全部門・全科目の一括 Excel 出力
- 指定フォーマット（繰越残高 → 案件別 → 当月累計）
- ダウンロード機能
...
- **テーブルエリア**: 前月繰越～案件分類/仕訳～当月残高を縦スクロール表示
  - Card ラッパーは使用しない。テーブル直置き。
  - テーブル上の部門/科目名や対象年月のタイトル表示は行わない（ヘッダーの選択状態で代替）。
  - 右上に「全案件オープン/クローズ」トグル（shadcn/ui Switch）を配置。チェックで全案件展開、オフで全て折りたたみ。
...
- 除外・特別扱い:
  - `AM列` が「＊＊ 前月繰越 ＊＊」: 前月繰越として `carryOver` に集計（全半角混在対応）。
...
- 不一致の場合は「この部門・科目のデータは未生成です…」の空状態を表示し、集計値は0表示（繰越残高/当月累計）。
...
- テーブル: shadcn `Table`で「繰越残高 → 案件 → 仕訳 → 当月累計」を表現
  ...
  - 案件行はクリックで展開/折りたたみ（Collapsible: grid-template-rows のトランジション, 260ms）
  - 「繰越残高」「当月累計」のラベルは摘要列（3列目）に表示し、金額は残高列（最右列）に表示する。借方・貸方合計は当月累計行で従来どおり該当列に表示。
