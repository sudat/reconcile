...
export async function POST(req: Request) {
  try {
    ...
    if (!body) return new Response(JSON.stringify({ ok: false, error: "invalid_body" }), { status: 400 });
    if (!process.env.OPENAI_API_KEY) {
      return new Response(JSON.stringify({ ok: false, error: "missing_OPENAI_API_KEY" }), { status: 500 });
    }

    // トークン節約のため大型入力はサンプリング
    const sample = <T,>(arr: T[], max: number) => (arr.length > max ? arr.slice(0, max) : arr);
    const itemsA = sample(body.itemsA, 300);
    const itemsB = sample(body.itemsB, 300);
    const daySummary = sample(body.daySummary.filter((d) => d.diff !== 0), 62); // 2ヶ月分程度の上限

    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

    const sys = `あなたは日本語で回答する会計アナリストです。目的は「本支店勘定の元帳A/Bのアンマッチ結果」から、\n- どの日付・どの補助科目で、どちら側(A/B)がいくら過不足か\n- 典型的な原因の仮説（例: 日付ズレ、相手先ミス、逆仕訳、税額差）\n- 次に人が確認すべき観点\nを簡潔に要約することです。金額は千円区切りで表記してください。`;
    ...
    const user = {
      ...
      daySummary,
      examples: {
        a: itemsA.map(({ day, subAccountCode, subAccountName, amountSigned, voucherNo }) => ({
          day,
          sub: `${subAccountCode}:${subAccountName}`,
          amountSigned,
          voucherNo,
        })),
        b: itemsB.map(({ day, subAccountCode, subAccountName, amountSigned, voucherNo }) => ({
          day,
          sub: `${subAccountCode}:${subAccountName}`,
          amountSigned,
          voucherNo,
    ...
    const text = completion.choices[0]?.message?.content?.trim() || "要約を生成できませんでした。";
